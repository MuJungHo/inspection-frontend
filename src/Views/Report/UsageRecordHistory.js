...
const UsageRecordHistory = () => {
  ...
  const { parkingFacilityUsageRecordId } = useParams();
  const location = useLocation()

  const [filter, setFilter] = React.useState({ ...initFilter, parkingFacilityUsageRecordId });
  const [UsageRecordList, setUsageRecordList] = React.useState([]);
  const lastPage = location.pathname.split("/")[1];

  React.useEffect(() => {
    getUsageRecordHistory()
  }, [filter])

  const getUsageRecordHistory = async () => {
    const { data, total, success } = await authedApi.record.getUsageRecordHistory(filter);
...
export default UsageRecordHistory;