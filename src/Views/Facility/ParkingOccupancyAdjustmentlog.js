...
const NAME = "parking-occupancy-adjustmentlog";

const ParkingOccupancyAdjustmentlog = () => {
  const { t, authedApi,
    // openDialog,
    // closeDialog,
    // openSnackbar,
  ...
  React.useEffect(() => {
    getParkingOccupancyAdjustmentlog()
  }, [filter])

  const getParkingOccupancyAdjustmentlog = async () => {
    // setParkingOccupancyList([]);
    const { data, total, success } = await authedApi.parkingOccupancys.getParkingOccupancyAdjustmentlog({ ...filter, parkingOccupancyId });
...
export default ParkingOccupancyAdjustmentlog;